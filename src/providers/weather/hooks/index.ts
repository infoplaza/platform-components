...
export const useWeatherState = (
  ...
  const [element, setElement] = useState(config.element ?? DEFAULT_WEATHER_CONFIG.element)
  ...
  const [modelMember, setModelMember] = useState<string | null>(config.member ?? DEFAULT_WEATHER_CONFIG.member)
  const [modelLevel, setModelLevel] = useState<string | null>(config.level ?? DEFAULT_WEATHER_CONFIG.level)
...
export const useWeatherLayers = (
  ...
  const getUnit = DEFAULT_GET_UNIT
