...
export default function mesh(order = 4, uvMap = false) {
    ...
    let triangles: IndexArray = Uint16Array.of(
    ...
    let v = 12;
    const midCache = new Map<number, number>(); // midpoint vertices cache to avoid duplicating shared vertices

    function addMidPoint(a: number, b: number): number {
    ...
    let trianglesPrev: IndexArray = triangles;
    const IndexArrayCtor = order > 5 ? Uint32Array : Uint16Array;

    for (let i = 0; i < order; i++) { // repeatedly subdivide each triangle into 4 triangles
        const prevLen: number = trianglesPrev.length;
        triangles = new IndexArrayCtor(prevLen * 4);
    ...
    function addDuplicate(i: number, uvx: number, uvy: number, cached: boolean): number {
        if (cached) {
